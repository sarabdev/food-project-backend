...
async function allocationLines(connection, allocations, clientId) {
  const ids = [...new Set(allocations.map((item) => Number(item.export_order_item_id)))];
  if (ids.length !== allocations.length) {
    const error = new Error("Each sales contract line can be selected only once per shipment.");
    error.status = 409;
    throw error;
  }
  ...
  const requested = new Map(allocations.map((item) => [Number(item.export_order_item_id), Number(item.quantity)]));
  ...
  return lines.map((line) => ({ ...line, shipment_quantity: requested.get(Number(line.id)) }));
}
...
shipmentsRouter.get("/:id", requirePermission("orders.view"), asyncHandler(async (req, res) => {
  ...
  const [items] = await pool.execute(
    `SELECT sa.id, sa.line_number, sa.quantity, i.product_id, i.quantity_unit,
      ...
      COALESCE(o.sales_contract_number, o.invoice_number) AS contract_number
     FROM shipment_allocations sa JOIN export_order_items i ON i.id = sa.export_order_item_id
     JOIN products p ON p.id = i.product_id JOIN export_orders o ON o.id = i.export_order_id
     WHERE sa.shipment_id = ? ORDER BY sa.line_number`,
    [req.params.id]
  );
  let cartonCursor = 1;
  const calculatedItems = items.map((item) => {
    ...
    if (cartons) cartonCursor = cartonEnd + 1;
    return {
  ...
  res.json({ shipment: { ...rows[0], items: calculatedItems } });
}));
  ...
  try {
    ...
    const [result] = await connection.execute(
      ...
      [
        ...
        input.shipping_type || null, input.shipped_per || null, input.container_number || null,
        input.container_type || null, input.cbm, input.freight_term || null, input.notes || null
      ]
    );
    for (const [index, allocation] of input.allocations.entries()) {
      await connection.execute(
        "INSERT INTO shipment_allocations (shipment_id, export_order_item_id, line_number, quantity) VALUES (?, ?, ?, ?)",
        [result.insertId, allocation.export_order_item_id, index + 1, allocation.quantity]
      );
    }
